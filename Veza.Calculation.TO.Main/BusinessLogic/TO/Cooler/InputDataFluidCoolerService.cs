...
{
    sealed public class InputDataFluidCoolerService : CalculateBaseService, IInputDataFluidCoolerService, IExtInputDataFluidCoolerService
    {
        #region Внутренние поля и переменные

        private readonly IFluidCoolerMapper fluidMapper;
        /// <summary>
        ...
        #region Конструктор
        public InputDataFluidCoolerService(AdvanceParamsViewModel advanceParamsViewModel, IInputData inputData, ICalculate calculate,
            ILogs logs, ITableDataService tableDataService, IInputDataService inputDataService,
            IFinThk finThk, IFluidCoolerMapper fluidMapper) :
        ...
        public bool Calc()
        {
            ...
            {
                case "direct":
                    CalcDirect(Calculation.TO.Main.Properties.Resources.PageFluidCooler_Direct);
                    break;
                case "reverse":
                    CalcReverse(Calculation.TO.Main.Properties.Resources.PageFluidCooler_Reverse);
                    break;
            }
            return IsCalc = true;
        }
        ...
        public OutputDataFluidHeaterCoolerDTO GetResults()
        {
            ...
            inputData.CalcFluidCooler = true;
            calculate.End();
        ...
        protected override void SetPropToCalc_Table(OutView outView)
        {
            foreach (var item in lines)
            {
                ...
                {
                    item.Content = outView.I_AirTempInCW.ToString("G", CultureInfo.InvariantCulture);
                    continue;
                }
                // владность воздуха на входе
                if (item.Field == "I_AirHumIn")
                {
                    item.Content = outView.I_AirHumInCW.ToString("G", CultureInfo.InvariantCulture);
                    continue;
                }
                ...
                {
                    item.Content = outView.I_AirflowCW.ToString("G", CultureInfo.InvariantCulture);
                    continue;
                }
                ...
                {
                    item.Content = outView.I_MedTempInCW.ToString("G", CultureInfo.InvariantCulture);
                    continue;
                }
                // Температура теплоносителя на выходе
                if (item.Field == "I_MedTempOut")
                {
                    item.Content = outView.I_MedTempOutCW.ToString("G", CultureInfo.InvariantCulture);
                    continue;
                }
        ...
        /// <summary>
        /// Установка свойств специфичных для прямого расчёта - воздухоохладитель
        /// </summary>
        private void SetPropToCalcCooler_Direct()
        {
            foreach (var item in lines)
            {
                if (inputData.SelectedVariantRasch == Calculation.TO.Main.Properties.Resources.I_TempFluidOut)
                {
                    ...
                    if (item.Field == "I_MedTempOut")
                    {
                        item.Content = inputData.ValueVariantRaschCW.ToString("G", CultureInfo.InvariantCulture);
                        continue;
                    }
                ...
                else
                {
                    ...
                    if (item.Field == "I_MedFlow")
                    {
                        item.Content = inputData.I_MedFlowCW.ToString("G", CultureInfo.InvariantCulture);
                        continue;
                    }
                    // Температура теплоносителя на выходе
                    if (item.Field == "I_MedTempOut")
                ...
                if (item.Field == "I_Capacity")
                {
                    ...
                    {
                        item.Content = inputData.ValueCapacityCW.ToString("G", CultureInfo.InvariantCulture);
                        continue;
                    }
                    else
                    {
                ...
                if (item.Field == "I_AirTempOut")
                {
                    ...
                    {
                        item.Content = inputData.DirectAirTempOutCW.ToString("G", CultureInfo.InvariantCulture);
                        continue;
                    }
                    else
                    {
                ...
                {
                    if (inputData.ValueMedKPaCW == 0)
                        item.Content = "40";
                    else item.Content = inputData.ValueMedKPaCW.ToString("G", CultureInfo.InvariantCulture);
                    continue;
                }
                ...
                {
                    item.Content = inputData.I_Type = "4";
                    continue;
                }
        ...
        private void SetPropToCalcCooler_Reverse()
        {
            foreach (var item in lines)
            {
                if (inputData.SelectedVariantRasch == Calculation.TO.Main.Properties.Resources.I_TempFluidOut)
        ...
        private void SetPropToCalcHeatCooler()
        {
            foreach (var item in lines)
            {
                ...
                if (item.Field == "I_MedTempIn")
                {
                    item.Content = inputData.ProjectsOut.I_MedTempInStrCW = inputData.ValueMedTempInCW.ToString("G", CultureInfo.InvariantCulture);
                    inputData.ProjectsOut.I_MedTempInStrCW += " " + Calculation.TO.Main.Properties.Resources.degreeСelsius;
                    continue;
                }
        ...
        /// <summary>
        /// установка входных данных специфичных для прямого расчёта - воздухоохладитель
        /// </summary>
        protected override void SetPropToCalc_DirectBase()
        {
            SetPropToCalc();
            SetPropToCalcHeatCooler();
            SetPropToCalcCooler_Direct();
        }
        ...
        protected override void SetSelectHeatExchanger(SaveCodes saveCodes)
        {
            ...
            inputData.ValueCapacityCW = saveCodes.CapacityCW;
            // Расход воздуха
            inputData.ValueAirFlowCW = saveCodes.ValueAirFlowCW;
            // Температура воздуха на входе
            inputData.I_AirTempInCW = saveCodes.I_AirTempInCW;

            // Влажность воздуха на входе
            inputData.ValueBaseHumCW = saveCodes.ValueBaseHumCW;
            // Температура теплоносителя на входе
            inputData.ValueMedTempInCW = saveCodes.ValueMedTempInCW;

            // Максимальное падение давления
            inputData.ValueMedKPaCW = saveCodes.ValueMedKPaCW;
            // Вариант расчёта 
            inputData.ValueVariantRaschCW = saveCodes.ValueVariantRaschCW;
        ...
        protected override void SetSelectHeatExchangerPropToCalc(DBLine item)
        {
            ...
            if (item.Field == "I_Airflow")
            {
                inputData.ProjectsOut.I_AirflowCW = inputData.ValueAirFlowCW;
                item.Content = inputDataService.GetString_ValueAirFlowCW();
            }
            // владность воздуха на входе
            if (item.Field == "I_AirHumIn")
            {
                inputData.ProjectsOut.I_AirHumInCW = inputData.ValueBaseHumCW;
                item.Content = inputData.ValueBaseHumCW.ToString("G", CultureInfo.InvariantCulture);
            }
            // температура воздуха на входе
            if (item.Field == "I_AirTempIn")
            {
                item.Content = inputData.ProjectsOut.I_AirTempInStrCW = inputDataService.GetString_I_AirTempInCW();
                inputData.ProjectsOut.I_AirTempInStrCW += " " + Calculation.TO.Main.Properties.Resources.degreeСelsius;
            }
        }
        ...
        /// <summary>
        /// Установка входных данных специфичных для теплоохладителя
        /// </summary>
        private void SetPropertiesCW()
        {
            if (!inputData.InitCW)
            {
                inputData.ValueBaseHumCW = 50;
                inputData.ValueMedKPaCW = 70;
                inputData.ValueMedTempInCW = 6;
                inputData.ValueVariantRaschCW = 12;
                inputData.I_AirTempInCW = 32;
                foreach (var item in TableData.ProjectsOutTable)
                {
                    item.I_AirHumInCW = 50;
                    item.I_MedTempInCW = 6;
                    item.I_MedTempOutCW = 12;
                    item.I_AirTempInCW = 32;
                    item.I_AirTempOutCW = "20";
                }
                inputData.ValueCapacityCW = inputData.ValueCapacityHW;
                inputData.ValueAirFlowCW = inputData.ValueAirFlowHW;
                inputData.InitCW = true;
            }
