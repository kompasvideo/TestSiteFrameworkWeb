...
{
    public class FluidCoolerMapper : IFluidCoolerMapper
    {
        ...
        public FluidCoolerMapper(AdvanceParamsViewModel advanceParamsViewModel, IInputDataService inputDataService, 
        ...
        /// Mode - тип расчёта : direct - прямой, reverse - обратный
        /// DirectMode - список (мощность, температура воздуха на выходе)
        /// ValueCapacityCW - значение DirectMode
        ...
        /// ValueAirFlowCW - расход воздуха
        /// I_AirTempInCW - температура воздуха на входе
        /// ValueBaseHumCW - влажность воздуха на входе
        /// SelectedFluid - теплоноситель
        /// I_SoleCnz - концентрация гликоля
        /// ValueMedTempInCW - температура теплоносителя на входе
        /// SelectedVariantRasch - список(режим расчёта) - температура теплоносителя на выходе или расход теплоносителя
        /// ValueVariantRaschCW - температура теплоносителя на выходе или расход теплоносителя
        /// ValueMedKPaCW - максимальное падение давления
        ...
        public void InputToInputDataClass(InputDataFluidHeaterCoolerDTO inputParams)
        {
            ...
            inputDataService.SetValueCapacityCW(inputParams.ValueCapacity);
            ...
            inputDataService.SetValueAirFlowCW(inputParams.ValueAirFlow);
            inputDataService.SetI_AirTempInCW(inputParams.I_AirTempIn);
            inputDataService.SetValueBaseHumCW(inputParams.ValueBaseHum);
            inputData.SelectedFluid = inputParams.SelectedFluid;
            inputDataService.SetI_SoleCnz(inputParams.I_SoleCnz);
            inputDataService.SetValueMedTempInCW(inputParams.ValueMedTempIn);
            inputData.SelectedVariantRasch = inputParams.SelectedVariantRasch;
            inputDataService.SetValueVariantRaschCW(inputParams.ValueVariantRasch);
            inputDataService.SetValueMedKPaCW(inputParams.ValueMedKPa);
        ...
        public GetDataFluidHeaterCoolerDTO InputDataClassToInputParams()
        {
            return new GetDataFluidHeaterCoolerDTO()
            {
                ...
                // характеристики корпуса и коллектора
                I_HdrQta = inputDataService.GetI_MatHdr(),
